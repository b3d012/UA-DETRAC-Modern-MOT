...
def main(arguments: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--split", type=Path, default=PROJECT_ROOT / "data/splits/ua_detrac_m3_split.json"
    )
    parser.add_argument("--mode", choices=("tuning", "evaluation"), required=True)
    parser.add_argument(
        "--role", choices=("development_train", "validation", "official_test"), required=True
    )
